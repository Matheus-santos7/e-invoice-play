          ...
          return (
            <div key={i} className={`border rounded-lg p-5 flex gap-4 ${tones[ins.tone]}`}>
              ...
              <div className="flex-1">
                <div className="font-bold text-base">{ins.title}</div>
                <div className="text-[14px] text-muted-foreground mt-1">{ins.desc}</div>
              </div>
              <button
                type="button"
                className="text-[12px] font-bold uppercase tracking-widest text-accent hover:underline self-start"
              >
                {ins.action} →
              </button>
            </div>
